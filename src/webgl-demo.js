...
function main() {
  ...
  void main(void) {
    gl_FragColor = vColor;
//    gl_FragColor = vec4(vec3(vColor.r+vColor.g+vColor.b)/3.0, vColor.a);
  }
  `;

  const fsSource2 = `varying lowp vec4 vColor;
  ...
  function render(now) {
    ...
    if (counter % 30 === 0)
      lock = 1 - lock;
    if(lock === 1)
      drawScene(gl, programInfo2, buffers, deltaTime);    
    else
...
function initBuffers(gl) {
  ...
  const faceColors = [
    ...
    [0.0,  1.0,  1.0,  1.0],    // 
    [0.0,  0.5,  0.0,  1.0]    // Left face: purple
  ];
...
function drawScene(gl, programInfo, buffers, deltaTime) {
  ...
  mat4.translate(modelViewMatrix,     // destination matrix
                 ...
                 [-0.0, 0.0, -6.0+movementAmt]);  // amount to translate
